...
G_OPTIONAL_GENCB (struct vkq_gfamily, vkq_gfamily_set_cb,
		  value, opt);

struct vkqueues {
	GArray* 	   all;
	struct vkq_gfamily gfamily;
};
...
init_vkapp_queues (struct vkapp* p,
		   GError** 	 e)
{
	p->queues.all = vk_get_queue_family_props (p->pd_used);
	for (guint i = 0; i < p->queues.all->len; i++) {
		VkQueueFamilyProperties* iter;
		iter = &g_array_index (p->queues.all, VkQueueFamilyProperties, i);
		if (iter->queueFlags & VK_QUEUE_GRAPHICS_BIT) {
			g_optional_set (&p->queues.gfamily.opt, vkq_gfamily_set_cb, &i);
			break;
		}
	}
}
...
#endif
	g_optional_init (&p->queues.gfamily.opt);
...
term_vkapp (struct vkapp* p, GError** e)
{
	...
	g_array_free (p->queues.all, TRUE);
